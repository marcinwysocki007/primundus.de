import type { Metadata } from 'next'
import { ArticleCTA } from '@/components/ArticleCTA'
import { Weiterlesen } from '@/components/Weiterlesen'
import { ArticleProgressBar } from '@/components/ArticleProgressBar'
import { ArticleTOC } from '@/components/ArticleTOC'
import { AuthorByline } from '@/components/AuthorByline'
import { aktualisiertAm } from '@/lib/lastmod'
import { PERSON_MARTA_ID } from '@/lib/schema'

const AKTUALISIERT = aktualisiertAm('depression-im-alter-pflege', '30. April 2026')

const SECTIONS = [{ id: 'erkennen', title: 'Zeichen erkennen' }, { id: 'hilfe', title: 'Wie helfen' }, { id: 'behandlung', title: 'Behandlungsmöglichkeiten' }, { id: 'faq', title: 'Häufige Fragen' }]

export const metadata: Metadata = {
  title: 'Depression im Alter — Erkennen, Helfen, Pflegen | Primundus',
  description: 'Depression im Alter richtig erkennen und damit umgehen: Zeichen, wie die Betreuungskraft helfen kann und wann professionelle Hilfe nötig ist.',
  alternates: { canonical: 'https://primundus.de/depression-im-alter-pflege' },
  openGraph: {
    images: [{ url: '/images/og-default.jpg', width: 1200, height: 630 }],
    title: 'Depression im Alter',
    description: 'Depression im Alter richtig erkennen und damit umgehen: Zeichen, wie die Betreuungskraft helfen kann und wann professionelle Hilfe nötig ist.',
    url: 'https://primundus.de/depression-im-alter-pflege',
    siteName: 'Primundus',
    locale: 'de_DE',
    type: 'article',
  },
}

const schemaMarkup = [
  {
    '@context': 'https://schema.org',
    '@type': 'Article',
    headline: 'Depression im Alter — Erkennen, Helfen, richtig Pflegen',
    author: { '@id': PERSON_MARTA_ID },
    publisher: { '@type': 'Organization', name: 'Primundus', logo: 'https://primundus.de/images/primundus_logo_header.webp' },
    datePublished: '2026-04-30',
    dateModified: AKTUALISIERT.iso,
    mainEntityOfPage: 'https://primundus.de/depression-im-alter-pflege',
  },
  {
    '@context': 'https://schema.org',
    '@type': 'BreadcrumbList',
    itemListElement: [
      { '@type': 'ListItem', position: 1, name: 'Startseite', item: 'https://primundus.de/' },
      { '@type': 'ListItem', position: 2, name: 'Ratgeber', item: 'https://primundus.de/krankheiten' },
      { '@type': 'ListItem', position: 3, name: 'Depression im Alter', item: 'https://primundus.de/depression-im-alter-pflege' },
    ],
  },
  {
    '@context': 'https://schema.org',
    '@type': 'FAQPage',
    mainEntity: [
      { '@type': 'Question', name: 'Wie erkenne ich Depression im Alter?', acceptedAnswer: { '@type': 'Answer', text: 'Anhaltende Traurigkeit, Interessenverlust, Rückzug, Schlafprobleme, Appetitlosigkeit, häufige körperliche Beschwerden ohne klare Ursache.' } },
      { '@type': 'Question', name: 'Kann eine Betreuungskraft bei Depression helfen?', acceptedAnswer: { '@type': 'Answer', text: 'Ja — Strukturierung des Alltags, soziale Aktivierung, Begleitung bei Aktivitäten, konsequente Medikamentenerinnerung. Keine Therapie, aber wichtige Alltagsunterstützung.' } }
    ],
  },
]

export default function Page() {
  return (
    <>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(schemaMarkup) }} />
      <ArticleProgressBar />
      <ArticleTOC sections={SECTIONS} />
      <div className="min-h-screen bg-pm-paper">
        <div className="max-w-article mx-auto px-5 py-10 md:py-16">
          <nav className="min-h-[24px] text-sm text-pm-mute mb-6 flex items-center gap-2 flex-wrap">
            <a href="/" className="hover:text-pm-taupe transition-colors">Startseite</a>
            <span>›</span>
            <a href="/krankheiten" className="hover:text-pm-taupe transition-colors">Ratgeber</a>
            <span>›</span>
            <span className="text-pm-ink">Depression im Alter</span>
          </nav>
          <p className="text-meta font-bold uppercase tracking-[0.1em] text-pm-taupe-light mb-4">Krankheiten · 5 Min</p>
          <h1 className="text-h1 md:text-h1-lg font-bold text-pm-ink mb-6">Depression im Alter — Erkennen, Helfen, richtig Pflegen</h1>

          <AuthorByline updated={AKTUALISIERT.sichtbar} />
          <p className="text-[17px] md:text-[19px] leading-relaxed text-pm-body mb-10 font-medium">Depression im Alter wird oft übersehen — als 'Altersgriesgram' abgetan oder mit Demenz verwechselt. Dabei ist sie behandelbar. Eine gute Betreuungskraft kann entscheidend helfen.</p>
          <h2 id="erkennen" className="text-h2 md:text-h2-lg font-bold text-pm-ink mt-10 mb-4 leading-snug">Zeichen erkennen</h2>
          <p className="text-[16px] leading-relaxed text-pm-body mb-4">Dieser Ratgeber gibt Ihnen einen kompakten Überblick über das Thema — praxisnah und auf die Situation pflegender Familien zugeschnitten.</p>
          <h2 id="hilfe" className="text-h2 md:text-h2-lg font-bold text-pm-ink mt-10 mb-4 leading-snug">Wie helfen</h2>
          <p className="text-[16px] leading-relaxed text-pm-body mb-4">Die wichtigsten Informationen für Ihren Alltag mit Pflege: Was konkret zu tun ist, worauf man achten sollte und wie Primundus unterstützen kann.</p>
          <h2 id="behandlung" className="text-h2 md:text-h2-lg font-bold text-pm-ink mt-10 mb-4 leading-snug">Behandlungsmöglichkeiten</h2>
          <p className="text-[16px] leading-relaxed text-pm-body mb-4">Wenn Sie unsicher sind wie Sie vorgehen sollen — sprechen Sie mit uns. Primundus berät kostenlos und ehrlich: 089 200 000 830.</p>

          <h2 id="faq" className="text-h2 md:text-h2-lg font-bold text-pm-ink mb-6">Häufige Fragen</h2>
          <div className="space-y-4 mb-12">
            <details className="bg-white rounded-xl border border-pm-line group">
              <summary className="flex items-center justify-between px-5 py-4 cursor-pointer list-none">
                <h3 className="text-[15px] font-semibold text-pm-ink pr-4">Wie erkenne ich Depression im Alter?</h3>
                <span className="text-pm-taupe font-bold text-[20px] flex-shrink-0 group-open:rotate-45 transition-transform">+</span>
              </summary>
              <div className="px-5 pb-4"><p className="text-[15px] text-pm-body leading-relaxed">Anhaltende Traurigkeit, Interessenverlust, Rückzug, Schlafprobleme, Appetitlosigkeit, häufige körperliche Beschwerden ohne klare Ursache.</p></div>
            </details>
            <details className="bg-white rounded-xl border border-pm-line group">
              <summary className="flex items-center justify-between px-5 py-4 cursor-pointer list-none">
                <h3 className="text-[15px] font-semibold text-pm-ink pr-4">Kann eine Betreuungskraft bei Depression helfen?</h3>
                <span className="text-pm-taupe font-bold text-[20px] flex-shrink-0 group-open:rotate-45 transition-transform">+</span>
              </summary>
              <div className="px-5 pb-4"><p className="text-[15px] text-pm-body leading-relaxed">Ja — Strukturierung des Alltags, soziale Aktivierung, Begleitung bei Aktivitäten, konsequente Medikamentenerinnerung. Keine Therapie, aber wichtige Alltagsunterstützung.</p></div>
            </details>
          </div>
          <Weiterlesen aktuell="depression-im-alter-pflege" />
          <ArticleCTA />
        </div>
      </div>
    </>
  )
}
