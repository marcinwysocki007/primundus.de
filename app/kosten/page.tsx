...
const AKTUALISIERT = aktualisiertAm('kosten', '21. August 2026')
      ...
      <div className="min-h-screen bg-pm-paper">
        <div className="max-w-article mx-auto px-5 py-10 md:py-16">
          ...
          <nav className="min-h-[24px] text-sm text-pm-mute mb-6 flex items-center gap-2 flex-wrap">
            ...
            <span>›</span>
            <span className="text-pm-ink">Kosten</span>
          </nav>
          ...
          <ul className="space-y-2 mb-6">
            {[
              ...
              <li key={item} className="flex items-start gap-2.5 text-[15px] text-pm-body">
                <svg className="w-4 h-4 text-pm-taupe flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                </svg>
                {item}
              </li>
          ...
          <div className="bg-white rounded-2xl border border-pm-line overflow-hidden mb-6 shadow-sm">
            <div className="px-5 py-3 bg-pm-paper border-b border-pm-line">
              <p className="text-[12px] font-bold uppercase tracking-[0.08em] text-pm-mute">Beispielrechnung — Pflegegrad 3, 1 Person, Grundbetreuung</p>
            </div>
            <div className="divide-y divide-pm-line-soft">
              {[
                ...
                <div key={label} className="flex items-center justify-between px-5 py-3 bg-white">
                  ...
                  <span className={`text-[14px] font-semibold tabular-nums ${green ? 'text-pm-green' : 'text-pm-ink'}`}>{value}</span>
                </div>
              ))}
              <div className="flex items-center justify-between px-5 py-4 bg-pm-paper">
                <span className="text-[15px] font-bold text-pm-ink">Eigenanteil pro Monat</span>
                <span className="text-[18px] font-bold text-pm-ink">ca. 1.606 €</span>
              </div>
            ...
            <div className="px-5 py-3 border-t border-pm-line">
              <p className="text-[11px] text-pm-mute">Zzgl. Kost & Logis und Anreise der Betreuungskraft · Beispielrechnung 2026 · Ihr persönlicher Eigenanteil hängt von Ihrer Situation ab.</p>
            </div>
          </div>

          {/* Eigenanteil-Tabelle je Pflegegrad — zitierfähige Referenz mit Quellen */}
          <h3 className="text-[19px] md:text-[22px] font-bold text-pm-ink mt-2 mb-3 leading-snug">
                ...
                <tbody>
                  {[
                    ...
                    <tr key={pg} className={i % 2 === 0 ? 'bg-white' : 'bg-pm-paper'}>
                      <td className="px-3 py-3 text-[13px] font-semibold text-pm-ink border-b border-pm-line">{pg}</td>
                      <td className="px-3 py-3 text-[13px] text-pm-body border-b border-pm-line tabular-nums">{kosten}</td>
                      <td className="px-3 py-3 text-[13px] text-pm-green border-b border-pm-line tabular-nums">{pflegegeld}</td>
                      <td className="px-3 py-3 text-[13px] text-pm-green border-b border-pm-line tabular-nums">{budget}</td>
                      <td className="px-3 py-3 text-[13px] font-bold text-pm-ink border-b border-pm-line tabular-nums">{eigen}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <p className="text-[13px] text-pm-mute mb-10">
            Quellen:{' '}
            <a href="https://www.gesetze-im-internet.de/sgb_11/__37.html" target="_blank" rel="noopener noreferrer" className="text-pm-taupe underline">§ 37 SGB XI (Pflegegeld)</a>{' · '}
            <a href="https://www.gesetze-im-internet.de/sgb_11/__42a.html" target="_blank" rel="noopener noreferrer" className="text-pm-taupe underline">§ 42a SGB XI (gemeinsamer Jahresbetrag)</a>{' · '}
            <a href="https://www.gesetze-im-internet.de/sgb_11/__45b.html" target="_blank" rel="noopener noreferrer" className="text-pm-taupe underline">§ 45b SGB XI (Entlastungsbetrag)</a>{' · '}
            <a href="https://www.bundesgesundheitsministerium.de/themen/pflege/online-ratgeber-pflege/leistungen-der-pflegeversicherung/leistungen-im-ueberblick" target="_blank" rel="noopener noreferrer" className="text-pm-taupe underline">BMG-Leistungsübersicht</a>{' '}
            · Stand: August 2026
          </p>
          ...
          <div className="bg-white rounded-2xl border border-pm-line overflow-hidden mb-6 shadow-sm">
            <div className="px-5 py-3 bg-pm-paper border-b border-pm-line">
              <p className="text-[12px] font-bold uppercase tracking-[0.08em] text-pm-mute">Pflegegeld 2026 — alle Pflegegrade</p>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr className="bg-pm-paper">
                    {['Pflegegrad', 'Pflegegeld/Monat'].map(h => (
                      <th key={h} className="px-4 py-3 text-[12px] font-semibold text-pm-mute text-left border-b border-pm-line last:text-right">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {[
                    ...
                    <tr key={grad} className={i % 2 === 0 ? 'bg-white' : 'bg-pm-paper'}>
                      <td className="px-4 py-3 text-[14px] text-pm-body border-b border-pm-line">{grad}</td>
                      <td className="px-4 py-3 text-[14px] font-bold text-pm-green border-b border-pm-line text-right">{pg}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="px-5 py-2">
              <p className="text-[11px] text-pm-mute">Quelle: GKV-Spitzenverband · Stand 2026 · Keine Erhöhung für 2026, nächste Dynamisierung frühestens Jan. 2028</p>
            </div>
          </div>

          <div className="bg-pm-mint border border-[rgba(61,122,92,0.2)] rounded-2xl p-6 mb-6">
            <p className="text-[12px] font-bold uppercase tracking-[0.08em] text-pm-green-deep mb-2">Neu ab Juli 2025 — Entlastungsbudget</p>
            <p className="text-[15px] font-bold text-pm-ink mb-2">3.539 € pro Jahr — flexibel für Verhinderungs- und Kurzzeitpflege</p>
            <ul className="space-y-1.5">
              {[
                ...
                <li key={item} className="flex items-start gap-2 text-[14px] text-pm-green-deep">
                  <span className="text-pm-green font-bold mt-0.5 flex-shrink-0">✓</span>
                  {item}
                </li>
              ))}
            </ul>
          ...
          <p className="text-[15px] text-pm-body mb-3">
            ...
            <a href="/finanzierung" className="text-pm-taupe underline hover:text-pm-taupe-deep">Finanzierung der 24h-Pflege — vollständige Übersicht</a>
          </p>
          <p className="text-[15px] text-pm-body mb-10">
            → Pflegegeld im Detail:{' '}
            <a href="/pflegegeld" className="text-pm-taupe underline hover:text-pm-taupe-deep">Pflegegeld 2026 — Beträge & Anspruch</a>
          </p>
          ...
          <div className="bg-white border border-pm-line rounded-2xl overflow-hidden mb-4">
                ...
                <tbody>
                  {[['PG 2', '347 €', 'ca. 295 €', 'ab ca. 1.560 €/Monat'], ['PG 3', '599 €', 'ca. 295 €', 'ab ca. 1.310 €/Monat'], ['PG 4', '800 €', 'ca. 295 €', 'ab ca. 1.110 €/Monat'], ['PG 5', '990 €', 'ca. 295 €', 'ab ca. 920 €/Monat']].map(([pg, geld, budget, rest], i) => (
                    <tr key={pg} className={i % 2 === 0 ? 'bg-white' : 'bg-pm-paper'}>
                      ...
                      <td className="px-4 py-3 text-[13px] font-bold text-pm-green border-b border-pm-line">{geld}</td>
                      <td className="px-4 py-3 text-[13px] text-pm-green border-b border-pm-line">{budget}</td>
                      <td className="px-4 py-3 text-[13px] font-semibold text-pm-ink border-b border-pm-line">{rest}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="bg-pm-paper border-t border-pm-line px-4 py-3">
              <p className="text-[12px] text-pm-mute">Rechenbeispiel mit dem Einstiegspreis von 2.200 €/Monat; bei höherem Betreuungsbedarf entsprechend mehr. Dazu kommen Kost und Logis. Die Steuerermäßigung für haushaltsnahe Dienstleistungen (bis zu 333 €/Monat) senkt den Betrag zusätzlich.</p>
            </div>
          </div>
          <p className="text-[15px] text-pm-body mb-10">
            → Ihren persönlichen Preis samt Zuschüssen sehen Sie in 2 Minuten im{' '}
            <a href="https://kostenrechner.primundus.de/" className="text-pm-taupe underline hover:text-pm-taupe-deep">Kostenrechner</a>
          </p>
          ...
          <p className="text-[15px] text-pm-body mb-10">
            ...
            <a href="/pflegekraft-aus-polen" className="text-pm-taupe underline hover:text-pm-taupe-deep">Pflegekraft aus Polen — der große Ratgeber</a>
          </p>
          ...
          <div className="space-y-3 mb-6">
            {[
              ...
              <div key={item.title} className="bg-white rounded-xl p-5 border border-pm-line">
                ...
                {item.link && (
                  <a href={item.link.href} className="text-[13px] text-pm-taupe underline hover:text-pm-taupe-deep">→ {item.link.text}</a>
                )}
              </div>
            ))}
          </div>
          <p className="text-[15px] text-pm-body mb-10">
            → Alle Tipps kompakt:{' '}
            <a href="/eigenanteil-24h-pflege-senken" className="text-pm-taupe underline hover:text-pm-taupe-deep">Eigenanteil senken — alle Möglichkeiten 2026</a>
          </p>
          ...
          <div className="mt-12 mb-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="bg-white border border-pm-line rounded-xl p-4">
                ...
                <a href="/pflegesachleistungen" className="text-[13px] text-pm-taupe hover:underline block py-0.5">› Pflegesachleistungen</a>
              </div>
              ...
              <div className="bg-white border border-pm-line rounded-xl p-4">
                ...
                <a href="/sozialhilfe-bei-pflegebedarf" className="text-[13px] text-pm-taupe hover:underline block py-0.5">› Sozialhilfe bei Pflegebedarf</a>
              </div>
              ...
              <div className="bg-white border border-pm-line rounded-xl p-4">
                ...
                <a href="/kurzzeitpflege" className="text-[13px] text-pm-taupe hover:underline block py-0.5">› Kurzzeitpflege nutzen</a>
              </div>
