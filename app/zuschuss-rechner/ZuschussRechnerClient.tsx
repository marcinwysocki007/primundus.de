    ...
    <div className="min-h-screen bg-pm-paper">
      <div className="max-w-[720px] mx-auto px-5 py-14">
        ...
        <div className="bg-white border border-pm-line rounded-2xl p-6 shadow-sm mb-6">
          ...
          {!selected && (
            <p className="mt-3 text-[13px] text-pm-mute">
              Pflegegrad noch nicht bekannt?{' '}
              <a href="/pflegegrad-rechner" className="text-pm-taupe underline hover:text-[#7D6848]">Jetzt einschätzen →</a>
            </p>
          )}
        </div>
        ...
        {pg && (
          <>
            {/* Leistungsübersicht */}
            <div className="bg-white border border-pm-line rounded-2xl overflow-hidden shadow-sm mb-6">
              <div className="bg-pm-paper border-b border-pm-line px-5 py-3">
                <p className="text-[11px] font-bold uppercase tracking-[0.08em] text-pm-mute">
                ...
                {[
                  ...
                  <div key={row.label} className="px-5 py-3.5 flex items-start gap-4">
                    <span className={`w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 text-[10px] font-bold ${
                      row.active ? 'bg-pm-mint text-pm-green' : 'bg-pm-line-soft text-[#C0BAB2]'
                    }`}>
                      {row.active ? '✓' : '–'}
                    </span>
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[12px] text-pm-mute mt-0.5">{row.desc}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="bg-pm-shell border-t border-pm-line px-5 py-4">
                ...
                <p className="text-[12px] text-pm-taupe-ink">
            ...
            {/* Hinweis */}
            <div className="bg-pm-coral-tint border border-[rgba(231,111,99,0.2)] rounded-xl px-5 py-4 mb-8">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <a
                ...
                className="inline-flex items-center justify-center gap-2 bg-white border border-pm-taupe text-pm-taupe font-semibold text-[15px] py-3.5 px-6 rounded-xl hover:bg-pm-shell transition-colors"
              >
                089 200 000 830
              </a>
            </div>
